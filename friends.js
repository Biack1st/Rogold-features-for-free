...
pages.friends = async () => {
     ...
     const setRemoving = (friend) => {
          if (friend.querySelector("input")) {
               friend.querySelector("input").checked =
                    selectedForRemoval.includes(friend.id);
               return;
          }
          ...
          checkbox.setAttribute(
               "style",
               "height: 91px;width: 20px;border-radius: 50%;margin-right: auto;margin-left: auto;"
          );
          ...
          checkbox.addEventListener("click", (e) => {
               if (e.currentTarget.checked) {
                    ...
                    selectedForRemoval.splice(
                         selectedForRemoval.indexOf(
     ...
     let maxBF = await get(
          `https://inventory.roblox.com/v1/users/${await getUserId()}/items/GamePass/26817185`
     ); // Very mean if you change this ;( its cheap too
     let maxNum = maxBF.data && maxBF.data[0] ? 18 : 18;
     ...
     const friendLoad = async (friend) => {
          ...
          if (loadedList.includes(friend.getAttribute("id"))) {
               if (!friend.className.includes("rg-class")) {
                    loadedList.splice(
                         loadedList.indexOf(friend.getAttribute("id")),
                         1
                    );
               } else {
                    return;
          ...
          if (
               !friend.querySelector(".icon-more") &&
               doFavorites &&
               window.location.href.includes("users/friends#!/friends")
          ) {
            ...
            `;
               favoriteButton.setAttribute(
                    "style",
                    "right:5px;top:0;position:absolute;"
               );
          ...
          const disabled = friend.getElementsByClassName(
          ...
          if (status) {
               ...
               if (friendActivities.find((f) => f.id == id)) return;
               friendActivities.push({ id, friend, status, disabled });
          }
          // }
     };
     on(".list-item.avatar-card", friendLoad);
     setInterval(async () => {
          const activities = await getPresence(
               friendActivities.map((f) => f.id)
          );
          for (const activity of activities) {
               const friendInfo = friendActivities.find(
                    (f) => f.id == activity.userId
               );
               if (!friendInfo) continue;
               friendActivities.splice(friendActivities.indexOf(friendInfo), 1);
               if (
                    activity.lastLocation?.includes("Creating") &&
                    activity.placeId &&
                    activity.userPresenceType !== 0
               ) {
                    friendInfo.status.innerHTML = `<a class="text-link avatar-status-link" href="https://roblox.com/games/${
                         activity.placeId
                    }" title="${stripTags(activity.lastLocation)}">${stripTags(
                         activity.lastLocation
                    )}</a>
                `;
               } else if (activity.userPresenceType === 0) {
                    friendInfo.status.innerHTML = `${
                         (friendInfo.disabled && "Inactive") ||
                         stripTags(activity.lastLocation)
                    } - Seen ${
                         (activity.lastOnline &&
                              dateSince(new Date(activity.lastOnline))) ||
                         "Unknown"
                    }</div>
                `;
                    const avatarName = friendInfo.friend.getElementsByClassName(
                         "text-overflow avatar-name"
                    )[0];
                    if (
                         friendInfo.disabled &&
                         avatarName &&
                         avatarName.tagName == "DIV"
                    ) {
                         // actually disabled or banned
                         ...
                         replacement.textContent = stripTags(
                              avatarName.textContent
                         );
                         avatarName.parentNode.replaceChild(
                              replacement,
                              avatarName
                         );
                    }
     ...
     modal.setAttribute(
          "style",
          "position: fixed; z-index: 1002; height: 277px; width: 439px; left: 738px; top: 365px; display: none;"
     );
     modal.innerHTML = `
    <div role="dialog">
    ...
    `;
     ...
     confirm.addEventListener("click", async () => {
          ...
          for (const friend of selectedForRemoval) {
               const response = await postRepeat(
                    `https://friends.roblox.com/v1/users/${friend}/unfriend`,
                    {
                         headers: {
     ...
     const createBulkButton = async () => {
          ...
          bulkRemoveButton.setAttribute(
               "style",
               "padding: 6px;float: left;margin-left: 3px;margin-right: 2px;"
          );
          ...
          bulkRemoveButton.addEventListener("click", async () => {
               ...
               if (!isRemoving) {
                    ...
                    if (selectedForRemoval.length > 0) {
                         modal.querySelector(
                              ".modal-body"
                         ).innerHTML = `Are you sure you would like to remove <strong>${
                              selectedForRemoval.length
                         }</strong> friend${
                              selectedForRemoval.length > 1 ? "s" : ""
                         }?`;
