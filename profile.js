...
pages.profile = async () => {
	...
	let userStatus = (await getPresence([friendId]))?.[0]
    let userId = await GetUserInfo()
	userId = userId.userId
	setTimeout(async () => {
		first("button.btn-control-md", async e => {
			if (e.textContent == "" || !e.textContent) {
				await sleep(4000)
			}
			e = document.querySelector("button.btn-control-md")
			const setCancel = async () => {
				e.textContent = "Cancel Request"
		...
		getSetting("Follows You").then(isOn => {
			if (!isOn) return;
	...
	const isBTR = (await first("body")).getAttribute("data-btr-page") == "profile"
	if (!isBTR && userStatus?.lastOnline) {
		...
		clone.innerHTML = `
				<p class="text-label">Last Online</p><p class="text-lead">${userStatus.userPresenceType === 0 && dateSince(date) || "Now"}</p>
			`;
		(await first('.profile-stat')).parentNode.appendChild(clone)
	}
	for (const stat of document.querySelectorAll('.profile-stat')) {
		stat.style.width = "33%"
	}
	setTimeout(async () => {
		if (!isBTR && !isBanned) {
			const totalPrice = await cacheValue("outfitValue-" + friendId, async() => {
			...
			document.querySelector("#profile-current-wearing-avatar > .container-header > h2").innerHTML = `
				Currently Wearing | Outfit Cost <span class="icon-robux-28x28 roblox-popover-close"></span> ${addCommas(totalPrice)}
			`
		}
	}, 0);
	setTimeout(() => {
	...
	function failedLoad() {
		if (rapTab) {
			rapTab.innerHTML = ``
			for (element of qsa('.rbx-tab-heading')) {
	...
	if (!isBTR) {
		...
		rapPage.innerHTML = `
			<div class="profile-game ng-scope section">
				...
				<div ng-show="isGridOn" class="game-grid ng-isolate-scope" id="col-grid">
					<ul class="hlist game-cards" horizontal-scroll-bar="loadMore()"></ul>
				</div>
		...
		qs('.tab-content').appendChild(rapPage)
	} else {
	...
	pager.onset = (newPage) => {
		...
		for (const info of page) {
			...
			if (info.serialNumber) {
				...
				limited.setAttribute('style', 'width: 100%;position: absolute;top: 87.5%;')
				limited.innerHTML = `
	...
	if (!isBTR) {
		...
		const tabCollectibles = document.getElementById('tab-collectibles')
		tabCollectibles.addEventListener('click', () => {
			tabCollectibles.parentNode.className = "rbx-tab active"
		...
		if (window.location.href.includes("#collectibles")) {
			tabCollectibles.dispatchEvent(new Event('click'))
		}
		function locationHashChanged(e) {
			if (location.hash.includes("#collectibles") && !tabCollectibles.parentNode.className.includes("active")) {
				tabCollectibles.dispatchEvent(new Event('click'))
			}
		}
