                        ...
                        <profile-description-view description="$ctrl.data.description" class="ng-isolate-scope">
                            <div class="profile-about-content toggle-target">
                                <pre id="profile-about-text"
                                    ...
                                    <span class="profile-about-content-text ng-binding linkify" ng-class="{'linkify': !$ctrl.inApp}" ng-bind-html="$ctrl.description | linkify">${stripTags(userInfo.description)}</span></pre> 
                            </div>
...
white-space: nowrap;
        ...
        <li class="list-item game-card game-tile">
            <div class="game-card-container"><a
                    ...
                    <div class="game-card-info"><span class="info-label icon-votes-gray"></span> <span
                            class="info-label vote-percentage-label" id="${stripTags(favorite.Item.Name)}-rating">93%</span> <span class="info-label no-vote hidden"></span>
                        <span class="info-label icon-playing-counts-gray"></span> <span class="info-label playing-counts-label"
                            title="137" id="${stripTags(favorite.Item.Name)}-playing">137</span></div>
                </a></div>
        </li>
        `
	...
	for (const badge of rBadges) {
        ...
        title="${stripTags(badge.description)}">
        <span class="border asset-thumb-container icon-badge-${badge.name.toLowerCase().split(" ").join("-")}" title="${stripTags(badge.name)}"
        style="
        ...
        "
        ></span><span
            class="font-header-2 text-overflow item-name">${stripTags(badge.name)}</span></a>
        `
