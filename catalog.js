...
class InventoryHandler {
    constructor(userId) {
        ...
        this.cache.time = Date.now() + 1000 * 60 * 60 * 3
        this.cache.inventory = []
        this.cache.bundles = []
        this.cache = JSON.parse(localStorage.getItem("inventoryCache")) || this.cache
    ...
    async fillInventory() {
        let inventory = await get(`https://inventory.roblox.com/v2/users/${this.userId}/inventory?assetTypes=${this.assetTypes.join(",")}&limit=100&sortOrder=Asc${this.cache.cursor && "&cursor=" + this.cache.cursor || ""}`)
        this.cache.inventory = await this.uniq(this.cache.inventory.concat(inventory.data?.map(asset => asset.assetId)))
        this.cache.cursor = inventory.nextPageCursor
        if (inventory.nextPageCursor) {
            this.fillInventory()
        } else {
            this.cache.time = Date.now() + 1000 * 60 * 60 * 3
            // chrome.storage.local.set({ inventoryCache: this.cache })
            localStorage.setItem("inventoryCache", JSON.stringify(this.cache))
        }
    }

    async fillBundles() {
        let bundles = await get(`https://catalog.roblox.com/v1/users/${this.userId}/bundles?sortOrder=Asc&limit=100${this.cache.bundleCursor && "&cursor=" + this.cache.bundleCursor || ""}`)
        this.cache.bundles = await this.uniq(this.cache.bundles.concat(bundles.data?.map(bundle => bundle.id)))
        this.cache.bundleCursor = bundles.nextPageCursor
        if (bundles.nextPageCursor) {
            this.fillBundles()
        } else {
            this.cache.time = Date.now() + 1000 * 60 * 60 * 3
            localStorage.setItem("inventoryCache", JSON.stringify(this.cache))
    ...
    async getBundles() {
        if (this.cache.bundles.length == 0) {
            await this.fillBundles()
        }
        return this.cache.bundles
    }

    async getInventory() {
        if (!this.cache.inventory?.length || this.cache.time < Date.now()) {
            this.cache.inventory = []
            this.cache.bundles = []
    ...
    async addBundle(assetId) {
        let bundles = await this.getBundles()
        if (!bundles.includes(assetId)) {
            bundles.push(assetId)
            localStorage.setItem("inventoryCache", JSON.stringify(this.cache))
        }
    }

    async removeItem(assetId) {
        let inventory = await this.getInventory()
        if (inventory.includes(assetId)) {
            inventory.splice(inventory.indexOf(assetId), 1)
            localStorage.setItem("inventoryCache", JSON.stringify(this.cache))
        }
    }

    async hasItem(assetId) {
        let inventory = await this.getInventory()
        return inventory.find(id => id == assetId) !== undefined;
    }

    async hasBundle(assetId) {
        let bundles = await this.getBundles()
        return bundles.find(id => id == assetId) !== undefined;
    }
}
pages.catalog = (async () => {
    setTimeout(async () => {
        ...
        if (window.location.href.includes("catalog?Category=")) return;
        const productInfo = await get(`https://economy.roblox.com/v2/assets/${id}/details`)
    ...
    setTimeout(async () => {
        if (window.location.href.includes("/catalog?Category") || window.location.href.split("/")[3] == "catalog") {
            ...
            const panelGroup = isMobile() && (await first(".mobile-panel #category-panel-group")) || (await first("#main-view #category-panel-group"))
            const el = document.createElement('li')
            el.setAttribute('class', 'font-header-2 text-subheader panel panel-default')
            if (isMobile()) {
                el.innerHTML = `
                    <a data-target="#mobile-category-15" id="category-rogold" class="panel-heading ng-binding font-bold" data-toggle="collapse" data-parent="#category-panel-group" role="tab">
                        <span class="icon-checkmark-blue selected-icon"></span> 
                        RoGold
                    </a> 
                    `
            } else {
            ...
            el.addEventListener("click", async () => {
                if (didOpen) return;
                didOpen = true
                for (const button of (isMobile() && qsa("#category-panel-group li") || qsa("#main-view .btn-text"))) {
            ...
            if (await getSetting("Quick Buy")) {
                const handler = new InventoryHandler(await getUserId())
                handler.getInventory()
                handler.getBundles()
                on(".item-card-link .thumbnail-2d-container", async e => {
                    const cardLink = e.closest("a.item-card-container")
                    const buyButton = document.createElement('button')
                    buyButton.className = "ng-hide btn-full-width btn-common-play-game-lg btn-primary-md btn-min-width"
                    buyButton.style = `position: absolute;width: 30%;min-width: 37%;padding:2px;margin-left:2px;bottom: 0;margin-bottom: 2px;
                    display: flex;align-items: center;justify-content: center;transition: all 1s;z-index: 5;`
                    buyButton.innerHTML = `<span class="group-menu-item-text" style="line-height: 1.5;font-size: small;">Get</span>`
                    e.before(buyButton)
                    const itemId = Number(e.getAttribute("thumbnail-target-id"))
                    const modal = new Modal(itemId, "Buy Item", "Placeholder", "Placeholder")
                    cardLink.addEventListener("mouseleave", () => {
                        buyButton.classList.add("ng-hide")
                    })
                    cardLink.addEventListener("mouseenter", async () => {
                        buyButton.classList.remove("ng-hide")
                        const isBundle = cardLink?.href?.includes("bundles")
                        if (isBundle ? (await handler.hasBundle(itemId)) : (await handler.hasItem(itemId))) {
                            buyButton.innerHTML = `<span class="group-menu-item-text" style="line-height: 1.5;font-size: small;">Owned</span>`
                    ...
                    buyButton.addEventListener("click", async (ev) => {
                        ev.preventDefault()
                        const isBundle = cardLink?.href?.includes("bundles")
                        if (isBundle ? (await handler.hasBundle(itemId)) : (await handler.hasItem(itemId))) return;
                        let itemInfo
                        if (isBundle) {
                            let bundleInfo = await get(`https://catalog.roblox.com/v1/bundles/${itemId}/details`)
                            itemInfo = {
                                priceStatus: bundleInfo.product.isFree ? "Free" : "For Sale",
                                price: bundleInfo.product.priceInRobux || 0,
                                name: bundleInfo.name,
                                creatorName: bundleInfo.creator.name,
                                productId: bundleInfo.product.id,
                            }
                        } else {
                            itemInfo = (await postRepeat("https://catalog.roblox.com/v1/catalog/items/details", {
                                data: JSON.stringify({
                        ...
                        if (itemInfo) {
                            if (itemInfo.priceStatus == "Offsale") return;
                            const getType = itemInfo.priceStatus == "Free" && "Get" || "Buy"
                            const price = itemInfo.lowestPrice || itemInfo.price
                            modal.title = getType + " Item"
                            modal.content = `Would you like to ${getType.toLowerCase()} the item "<strong>${itemInfo.name}</strong>" from ${itemInfo.creatorName} for 
                            ${
                                itemInfo.priceStatus == "Free" ? "<strong>Free</strong>" : 
                                `<span class="icon-robux-16x16"></span><span class="text-robux">${addCommas(price)}</span>`
                            }?
                            <div class="modal-image-container roblox-item-image" data-image-size="medium" data-no-overlays="" data-no-click="">
                                <div style="position: relative; overflow: hidden;">
                                    <img title="${itemInfo.name}" alt="${itemInfo.name}" border="0" class="original-image modal-thumb" src="${qs("img", e).src}">
                                </div>
                            </div>
                            `
                            modal.footer = `This cannot be undone.`
                            modal.removeButton(itemId + "-confirm-btn")
                            modal.addButton(getType + " Now", itemId + "-confirm-btn", (btn) => {
                                btn.addEventListener("click", async () => {
                                    if (itemInfo.priceStatus === "Offsale") return;
                                    let purchased
                                    if (itemInfo?.itemRestrictions?.find(e => e.includes("Limited"))) {
                                        const reseller = (await get("https://economy.roblox.com/v1/assets/" + itemId + "/resellers?limit=100"))?.data?.[0]
                                        if (reseller) {
                                            purchased = await postRepeat("https://economy.roblox.com/v1/purchases/products/" + itemInfo.productId, {
                                                data: JSON.stringify({
                                                    expectedCurrency: 1,
                                                    expectedPrice: reseller.price,
                                                    expectedSellerId: reseller.seller.id,
                                        ...
                                        purchased = await postRepeat("https://economy.roblox.com/v1/purchases/products/" + itemInfo.productId, {
                                            data: JSON.stringify({ expectedCurrency: 1, expectedPrice: price })
                                        })
                                    }
                                    if (purchased?.reason === "Success" || purchased?.reason === "AlreadyOwned") {
                                        console.log("bought")
                                        modal.hide()
                                        if (isBundle) handler.addBundle(itemId)
                                        else handler.addItem(itemId)
                                    }
                                })
                            }, true)
                            modal.show()
    ...
    const doSearch = async () => {
        return new Promise(async (resolve, reject) => {
            ...
            const searchCategory = assetType || "Shirt"
            let similars = []
            let startTime = Date.now()
            progressRequest(`https://catalog.roblox.com/v1/search/items?category=Clothing&keyword=${searchName}&limit=100&subcategory=Classic${searchCategory}`, level * 2, (level * 2) - 1, async (search, page) => {
                if (!search?.data) {
                    resolve([])
                }
                let searchArray = []
    ...
    const svgPath = chrome.runtime.getURL('/svg/icons.svg')
    let resultsCategory
